...
readcounts <- read.table("bazo_out.count", header = TRUE, sep="\t")
...
readcounts <- readcounts [,c(2:8)]
#readcounts
#head(readcounts,n=3)


names(readcounts) <- c("bazo_colitis_1","bazo_colitis_2","bazo_colitis_3","bazo_colitis_4","bazo_control_1","bazo_control_2","bazo_control_3")
...
colData(DESeq.ds)$condition <- relevel(colData(DESeq.ds)$condition,"bazo_control")
...
DGE.results <-results(DESeq.ds, independentFiltering = TRUE , alpha = 0.9)
...
#write.csv(resdata, file="bazo_deseq.csv", sep='\t')
...
DGEgenes
lista_genes <- c('ENSMUSG00000098343', 'ENSMUSG00000026656', 'ENSMUSG00000030688', 'ENSMUSG00000071291', 'ENSMUSG00000018796', 'ENSMUSG00000026193', 'ENSMUSG00000037706', 'ENSMUSG00000116972', 'ENSMUSG00000083563', 'ENSMUSG00000032554', 'ENSMUSG00000040466', 'ENSMUSG00000002985', 'ENSMUSG00000001249')

##extract the normalized read counts for DE genes into a matrix
#hm.mat_DGEgenes <- log.norm.counts[DGEgenes,]
hm.mat_DGEgenes <- log.norm.counts[lista_genes,]
...
library(edgeR)
sample_info.edger <-factor(c(rep("colitis",4), rep("control",3)))
...
# remove genes that do not have one count per million in the 7 samples

keep <- rowSums(cpm(edgeR.DGElist)>= 1)>=7
...
write.csv(DGE.results_edgeR, file="bazo_edgeR.csv", sep='\t')
...
DGE.results_edgeR
#EnhancedVolcano(DGE.results_edgeR, lab=rownames(DGE.results_edgeR), x="logFC", y = "FDR", xlim = c(-10,10), pCutoff = 0.05, ylab="Padj")
...
DGE.results_limma <- topTable(voomed.fitted, coef = "sample_info.edgercolitis",
                                    ...
                                    sort.by = "P")
write.csv(DGE.results_limma, file="bazo_limma.csv", sep='\t')
